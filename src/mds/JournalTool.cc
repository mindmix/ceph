...
int JournalTool::main(std::vector<const char*> argv)
{
  ...
  // Common args
  std::string rank_str;
  if(ceph_argparse_witharg(argv, i, &rank_str, "--rank", (char*)NULL)) {
    std::string rank_err;
...
int JournalTool::main_event(std::vector<const char*> argv)
{
  ...
  // Execute command
  // ===============
  JournalScanner js(rank, mdsmap->get_metadata_pool(), filter);
  if (command == "get") {
    r = js.scan();
    if (r) {
...
void EventOutputter::binary() const
{
  ...
  for (JournalScanner::EventMap::const_iterator i = scan.events.begin(); i != scan.events.end(); ++i) {
    ...
    std::string const path = path + "/" + filename.str();
